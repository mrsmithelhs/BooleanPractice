...
- Venn diagram coverage should confirm the overlapping-circle renderer exposes the canonical region ids exactly once through SVG region controls, the detailed-label toggle hides and reveals region labels without changing ids or selections, the fallback region list remains available, and proof views show paired visual diagrams for left/right or original/guess comparisons.
